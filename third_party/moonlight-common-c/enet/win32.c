...
#if !(defined(WINAPI_FAMILY) && WINAPI_FAMILY == WINAPI_FAMILY_APP) && (!defined(_WIN32_WINNT) || _WIN32_WINNT >= 0x0600)
# define HAS_QWAVE
...
typedef INT (WSAAPI *LPFN_WSASENDMSG)(SOCKET s, LPWSAMSG lpMsg, DWORD dwFlags, LPDWORD lpNumberOfBytesSent, LPWSAOVERLAPPED lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
#ifndef WSAID_WSASENDMSG
#define WSAID_WSASENDMSG {0xa4464610,0xb045,0x11d1,{0x8a,0x3e,0x00,0xc0,0x4f,0xc9,0xe2,0x92}}
#endif
static LPFN_WSASENDMSG pfnWSASendMsg;
                  ...
                  size_t bufferCount)
{
    ...
    if (pfnWSASendMsg != NULL) {
        sendResult = pfnWSASendMsg (socket,
                                    & msg,
                                    0,
                                    & sentLength,
                                    NULL,
        ...
        sendResult = WSASendTo (socket,
                                (LPWSABUF) buffers,
                                (DWORD) bufferCount,
                                & sentLength,
                                0,
                                (struct sockaddr *) & peerAddress -> address,
                                (int) peerAddress -> addressLength,
                                NULL,
                                NULL);
    }
                     ...
                     size_t bufferCount)
{
    ...
    if (pfnWSARecvMsg != NULL) {
        recvResult = pfnWSARecvMsg (socket,
                                    & msg,
                                    & recvLength,
                                    NULL,
                                    NULL);
        ...
        DWORD flags = 0;
        int fromLen = peerAddress != NULL ? (int)sizeof(peerAddress->address) : 0;
        recvResult = WSARecvFrom (socket,
                                  (LPWSABUF) buffers,
                                  (DWORD) bufferCount,
                                  & recvLength,
                                  & flags,
                                  peerAddress != NULL ? (struct sockaddr *) & peerAddress -> address : NULL,
                                  peerAddress != NULL ? &fromLen : NULL,
                                  NULL,
                                  NULL);
        msg.namelen = fromLen;
    }
